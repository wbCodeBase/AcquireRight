...
export default function Header() {
  ...
  return (
    <header className={`${isScrolled ? "bg-gradient-to-r from-[#fffafa] to-[#e8e0e0]" : "bg-gradient-to-r from-[#fffafa] to-[#e8e0e0]"} font-nunito-sans sm:sticky sm:top-0 left-0 w-full z-40`}>
      <div className="sm:max-w-6xl w-full mx-auto">
      ...
      {/* Mobile Menu */}
      <nav
        className={`lg:hidden fixed left-0 right-0 bg-gradient-to-r from-[#fffafa] to-[#e8e0e0] shadow-lg transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'
          }`}
        style={{ top: isScrolled ? '80px' : '96px' }}
      >
